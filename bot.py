...
async def on_message(message):
    ...
    if msg.startswith("$Run"):
        try:
            ...
            time.sleep(2)
            url = (
                "https://api.codechef.com/ide/status?link="
                + code["result"]["data"]["link"]
            )
            ...
            try:
                myembed.add_field(
                    name="Input", value=data["result"]["data"]["input"], inline=False
                )
            except:
                myembed.add_field(name="Input", value="s", inline=False)
            try:
                myembed.add_field(
                    name="Output", value=data["result"]["data"]["output"], inline=False
                )
            except:
                myembed.add_field(name="Output", value="s", inline=False)
            try:
                myembed.add_field(
                    name="memory", value=data["result"]["data"]["memory"], inline=False
                )
            except:
                myembed.add_field(name="memory", value="s", inline=False)
            try:
                myembed.add_field(
                    name="time", value=data["result"]["data"]["time"], inline=False
                )
            except:
    ...
    if msg.startswith("$help"):
        ...
        myembed.add_field(
            name="$csupcoming",
            value="To Get The Info About Upcoming Codechef Contest\nEg. $csupcoming",
            inline=False,
        )
        myembed.add_field(
            name="$cspresent",
            value="To Get The Info About Present Codechef Contest\nEg. $cspresent",
            inline=False,
        )
        myembed.add_field(
            name="$csuser",
            value="To Get The Basic Info About Codechef User\nEg. $csuser gkcs",
            inline=False,
        )
        myembed.add_field(
            name="$cscountproblem",
            value="To Get The Total Problem in CodeChef with respective of there complexity\nEg. $cscountproblem",
            inline=False,
        )
        myembed.add_field(
            name="$csproblem",
            value="To Get The Info About The Problem Solved By Codechef User\nEg. $csproblem priyansh19077",
            inline=False,
        )
        myembed.add_field(
            name="$cfuser",
            value="To Get The Info About The Codeforces User\nEg. $cfuser tourist",
            inline=False,
        )
        myembed.add_field(
            name="$cfupcoming",
            value="To Get The Info About Upcoming Codeforces Contest\nEg. $cfupcoming",
            inline=False,
        )
        myembed.add_field(
            name="$Run",
            value="""Virtual Compiler For Python\nEg.1)Without $Input\n$Run import json
x = {
  "name": "John",
  "age": 30,
  "city": "New York"
...
y = json.dumps(x)
print(y)\n2)With $Input\n$Run INDIA=int(input()) 
for i in range(INDIA):print('Happy Independence Day') $Input 5""",
            inline=False,
        )
        myembed.add_field(
            name="$C+Run",
            value="""Virtual Compiler For C++\nEg.1)Without $Input\n$C+Run #include <iostream>
using namespace std;
int main()
{
    for (int i=0;i<5;i++)
